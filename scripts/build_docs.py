...
def page(a) -> str:
    ...
    add("<h2>Secrets</h2>")
    fired = sum(a.redaction_kinds.values())
    add(
        ...
        "no code with it and reads bytes rather than text, so one NUL byte cannot blind "
        "it to a whole file.</p>"
    )
    add(
        f"<p>This fixture is secret-free by construction, and the redactor fired {fired} "
        "times on it. Redaction is exercised against a separate fixture of twenty "
        "credential-carrying commands, where the test compares against the exact values "
        "planted rather than asking whether the output looks clean.</p>"
    )
    add(
